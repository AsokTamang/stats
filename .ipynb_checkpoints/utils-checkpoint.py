...
def calculate_feature_importance(features,lr,X_test,y_test):  #X_test is the list of testing features
    if len(features)>0:
        ...
        for i in ordered_imp_means:
            ...
            result.update({name:[importance_value]})  #inorder to convert the dictionary into df , we must use the list for columns
        most_important = list(X_test.columns)[ordered_imp_means[0]]  #feature with the highest importance
...
def plot_happiness(variable, x1, y1, x2, y2):
    ...
    # Ploting the model's prediction
    ax.scatter(
        x2, y2,
